// ============================================
// COMPLETE CODE FOR: Universal Table Formatter Node - FINAL FIX
// ============================================
// Handles both direct summaries AND wrapped original_data
// ============================================

// Format list_tickets response
function formatListTickets(summaries) {
  console.log('═══════════════════════════════════════');
...
function formatGetTicket(summaries) {
  console.log('═══════════════════════════════════════');
...
if (summaries.length > 0) {
  switch (action) {
    case 'get_ticket':
      formattedOutput = formatGetTicket(summaries);
      break;

    case 'list_tickets':
      formattedOutput = formatListTickets(summaries);
      break;
  ...
  // No summaries found
  if (inputData.error) {
    formattedOutput = `⚠️  ${inputData.error}\n\nPlease try a different query or check if there are any tickets available.`;
  } else {
    formattedOutput = '📋 No tickets found matching your criteria.';
  }
}
